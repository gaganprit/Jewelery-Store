...
$form_password = $_POST['txtpassword'];
	...
	$sql = "SELECT user_id, username, password, ac_type, user_status FROM `users` WHERE username = '$form_user' ";
	...
	while ($row = $dbquery->fetcharray())
	{
		...
		if(($status == "0") AND ($ac_type == "Administrator"))
		{
			...
			echo "<script>window.location.href='adminarea/adminhome';</script>";
		}
